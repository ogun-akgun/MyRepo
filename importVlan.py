...
# Bug in STD labs
#1.4 Adapted to Netbox 4.3.1 Jul 26,2025
...
nb = pynetbox.api('https://128.224.55.5', token='cbb5441f039967ed26cdf9c1add1c6c0b1db889c')
...
class InputForm1(ttk.Frame):
    ...
    def get_or_create_cluster(self,cluster,tenant,site):
        ...
        try:
            ...
            if 'STD' in str(cluster):
                type_obj = "standard-with-dedicated-storage-cluster"
                tags.append(int(nb.extras.tags.get(slug="standard").id))
            if 'DC' in str(cluster):
                ...
                if 'SC' in str(cluster):
                    ...
                    b=ss[len(a):]
                    sonuc = a.lower() + '-' + b[:2].rjust(2, '0')
                    #tags.append(int(nb.extras.tags.get(slug=sonuc).id()))
                    #tags.append(int(nb.extras.tags.get(slug=self.cltype.get().lower()).id))
    ...
    def modify_vlan_info(self):
        ...
        try:
            vlname = str(self.entryCName.get()).lower().replace(" ","-")

            vlan_group = nb.ipam.vlan_groups.get(slug=vlname)

            if self.chkd.get():

                update_data = {
                    "name": self.entryCName.get().upper(),
                    "vid_ranges": [(40,99)],
                    "slug": vlname
                }
                vlan_group.update(update_data)
